...
class _DetailsBlogPageState extends State<DetailsBlogPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      blogModel = ModalRoute.of(context)!.settings.arguments as BlogModel;
      debugPrint("blog Data ========> ${blogModel!.toJson()}");
      isLogin();

      if (blogModel != null) {
        compute(blogCommentProcess, blogModel!).then((value) {
          blogModel = value;
  ...
  void updateViewCount() async {
    await BlogService.updateBlogCount(blogModel!.id.toString());
    // if(status) {
    //   setState(() {
    //     blogModel!.count = blogModel!.count++;
    //   });
    // }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return directionality(
      ...
      appBar: appbar(
        ...
        onTapRightIcon: () {
          String link = "https://profitfromit.co.in/blog/${blogModel!.id}";
          if (Platform.isIOS) {
            shareBlog(
              context,
              blogModel!.title ?? '',
            ...
            String shareMsg = "${blogModel!.title}\n\n$link";
                      ...
                      children: [
                        ...
                        Hero(
                          ...
                          child: ClipRRect(
                              ...
                              child: fadeInImage(blogModel!.image ?? '',
                                  getSize().width, 210)),
                        ),
                        ...
                        if (blogModel!.comments?.isEmpty ?? true) ...{
                          Center(
                            child: emptyState(AppAssets.commentsEmptyStateSvg,
                                appText.noComments, appText.noCommentsDesc),
                          ),
                        } else ...{
                          // comments
                          ...List.generate(blogModel!.comments?.length ?? 0,
                              (index) {
                            return commentUi(blogModel!.comments![index], () {
                              BlogWidget.showOptionDialog(blogModel!.id!,
                                  blogModel!.comments![index].id!, userIsLogin);
                            });
                          }),
                        },
                ...
                AnimatedPositioned(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                          color: Colors.white,
                          boxShadow: [
                            boxShadow(Colors.black.withValues(alpha: .1),
                                blur: 15, y: -3)
                          ],
                      ...
                      child: button(
                          onTap: () {
                            BlogWidget.showReplayDialog(blogModel!.id!, null);
                          },
                          width: getSize().width,
                          height: 52,
