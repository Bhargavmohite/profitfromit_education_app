...
import 'package:webinar/common/deeplink/deep_link_content_result.dart';
...
class _SingleCoursePageState extends State<SingleCoursePage> with SingleTickerProviderStateMixin {
  ...
  getData() async {
    ...
    await AppData.removeCourseNotification();
    setState(() {
      isLoading = true;
      showCourseNotification = false;
    });
    ...
    try {
      isBundleCourse = courseData != null ? courseData?.type == 'bundle' : (ModalRoute
          .of(context)!
          .settings
          .arguments as List)[1];
    } catch (_) {} finally {
      isBundleCourse = false;
    }
    try {
    ...
    try {
      lessonId = (ModalRoute.of(context)!.settings.arguments as List)[4];
    } catch (_) {}
    ...
    debugPrint("course id ===============> $id");
    debugPrint("lesson id ===============> $lessonId");
    debugPrint("courseData ===============> ${courseData?.toJson()}");
  ...
  getContent() async {
    ...
    if (lessonId != null && contentData != null) {
      if(courseData!.authHasBought == true) {
        final result = findLessonById(contents: contentData, lessonId: int.parse(lessonId.toString()));
        if (result != null) {
          final ContentItem item = result.item;
    ...
    Timer(const Duration(seconds: 2), () {
      for (var i = 0; i < (courseData?.comments.length ?? 0); i++) {
        if (commentId == courseData?.comments[i].id) {
          ...
          scrollController.animateTo((courseData!.comments[i].globalKey.findWidget ?? 0.0) > 230 ? (courseData!.comments[i].globalKey.findWidget ?? 0.0) - 230 : 0, duration: const Duration(milliseconds: 400), curve: Curves.linearToEaseOut);
        }
      }
                        ...
                        child: (token.isEmpty && (PublicData.apiConfigData?['webinar_private_content_status'] ?? '0') == '1')
                            ...
                            : (token.isNotEmpty && (PublicData.apiConfigData?['sequence_content_status'] ?? '0') == '1' && locator<UserProvider>().profile?.accessContent == 0)
                                ...
                                : NestedScrollView(
                                    ...
                                    headerSliverBuilder: (context, innerBoxIsScrolled) {
                                      return [
                                            ...
                                            child: Column(
                                              ...
                                              children: [
                                                if (courseData?.activeSpecialOffer != null) ...{
                                                  SpecialOfferWidget(courseData?.activeSpecialOffer?.toDate ?? 0, courseData?.activeSpecialOffer?.percent?.toString() ?? '0'),
                                                },
                                                ...
                                                if (courseData?.videoDemo != null) ...{
                                                  if (courseData?.videoDemoSource == 'youtube' || courseData?.videoDemoSource == 'vimeo') ...{
                                                    PodVideoPlayerDev(courseData?.videoDemo ?? '', courseData?.videoDemoSource ?? '', Constants.singleCourseRouteObserver),
                                                  } else ...{
                                                ...
                                                if ((courseData?.authHasBought == false) && (courseData?.cashbackRules.isNotEmpty ?? false)) ...{
                                                  space(16),
                                                  helperBox(AppAssets.walletSvg, appText.getCashback,
                                        ...
                                        SliverAppBar(
                                          ...
                                          shadowColor: Theme.of(context).scaffoldBackgroundColor.withValues(alpha: .2),
                                          elevation: 10,
                                          titleSpacing: 0,
                                    ...
                                    body: TabBarView(physics: const BouncingScrollPhysics(), controller: tabController, children: [
                                      // information page
                                      SingleCourseWidget.informationPage(courseData!, viewMore, () {
                                        setState(() {
                                          viewMore = !viewMore;
                                        });
                                      }, () => setState(() {}), bundleCourses: bundleCourses),

                                      // content page
                                      SingleCourseWidget.contentPage(courseData!, contentData, bundleCourses: bundleCourses, token: token),
                                      ...
                                      if (token.isNotEmpty) ...{
                                        Row(
                                          children: [
                                            ...
                                            const Spacer(),
                                            Text(
                                              ((courseData?.price ?? 0) == 0) ? appText.free : CurrencyUtils.calculator(courseData!.price ?? 0),
                                            ...
                                            if ((courseData!.discountPercent ?? 0) > 0) ...{
                                              space(0, width: 8),
                                              Text(
                                                CurrencyUtils.calculator((courseData!.price ?? 0) - ((courseData!.price ?? 0) * (courseData!.discountPercent ?? 0) ~/ 100)),
                                                ...
                                                child: button(
                                                    onTap: () async {
                                                      if (((courseData?.price ?? 0) == 0)) {
                                                        ...
                                                        bool res = isBundleCourse ? await PurchaseService.bundlesFree(courseData!.id!) : await PurchaseService.courseFree(courseData!.id!);
                                                      ...
                                                      if (courseData!.tickets.isNotEmpty || (courseData!.points != null && courseData!.points != 0)) {
                                                        SingleCourseWidget.pricingPlanDialog(courseData!).then((bool? value) {
                                                          if (value != null && value) {
                                                            courseData = null;
                                                            getData();
                                                        ...
                                                        bool status = await CartService.add(courseData?.id?.toString() ?? '', isBundleCourse ? 'bundle' : 'webinar', '');
                                      ...
                                      Column(
                                        ...
                                        children: [
                                          ...
                                          LayoutBuilder(
                                            builder: (context, constraints) {
                                              return Container(
                                                ...
                                                child: Container(
                                                  width: ((courseData?.progressPercent ?? 0) > 0) ? constraints.maxWidth * ((courseData?.progressPercent ?? 0) / 100) : 5,
                                                  height: 4,
                                          ...
                                          if (courseData?.type != 'bundle') ...{
                                            if (courseData?.meetingUrl != null && courseData?.meetingUrl != "") ...{
                                              button(
                                                  onTap: () {
                                                    launchUrlString(courseData!.meetingUrl!);
                                                  },
                                                  width: getSize().width,
                                                  height: 52,
                                                  text: "Meeting",
                                            ...
                                            if (courseData?.meetingNotification != null && courseData?.meetingNotification != "") ...{
                                              Text(
                                                courseData?.meetingNotification ?? "",
                                                style: style14Regular().copyWith(color: red49),
                                              ),
                                              space(12),
                                            },
                                          },
  ...
  DeepLinkContentResult? findLessonById({required List<ContentModel> contents, required int lessonId}) {
    try {
      for (int chapterIndex = 0; chapterIndex < contents.length; chapterIndex++) {
        final ContentModel chapter = contents[chapterIndex];
        final List<ContentItem> items = chapter.items ?? [];
        for (int itemIndex = 0; itemIndex < items.length; itemIndex++) {
          final ContentItem item = items[itemIndex];
          if (item.id == lessonId) {
            String? previousLink;
            ...
            if (previousIndex >= 0) {
              previousLink = items[previousIndex].link;
            }

            return DeepLinkContentResult(
              chapter: chapter,
              item: item,
      ...
      debugPrint("findLessonById Error ==========> $e");
      return null;
    }
  }
}
