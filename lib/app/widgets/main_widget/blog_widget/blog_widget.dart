...
class BlogWidget {
  static showOptionDialog(int postId, int? commentId, bool isLogin,
  ...
  static Future<bool?> showReplayDialog(int postId, int? commentId,
      {String itemName = 'blog'}) async {
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: [
              space(25),
              Text(
                commentId == null ? appText.comments : appText.replyToComment,
                style: style20Bold(),
              ),
              space(20),
              descriptionInput(
                ...
                child: button(
                    onTap: () async {
                      if (messageController.text.trim().isNotEmpty) {
                        ...
                        bool res = await BlogService.saveComments(postId,
                            commentId, itemName, messageController.text.trim());
                ...
                onTap: () {
                  if (messageController.text.trim().isNotEmpty) {
                    BlogService.reportComments(
                        commentId, messageController.text.trim());
  ...
  static Future showCategoriesDialog(
      BasicModel? selectedCategory, List<BasicModel> categories) async {
    return await baseBottomSheet(child: Builder(builder: (context) {
      return Padding(
        padding: EdgeInsets.only(
            bottom: MediaQuery.of(context).viewInsets.bottom,
                ...
                child: Container(
                  ...
                  child: Text(
                    ...
                    style: style16Regular().copyWith(
                        color: categories[index].id == selectedCategory?.id
                            ? green77()
                            : grey3A),
                  ),
                ),
