...
class LearningWidget {
        ...
        children: [
          ...
          ...List.generate(contents.length, (index) {
            return Container(
              ...
              decoration: BoxDecoration(color: Colors.white, borderRadius: borderRadius(radius: 15)),
              ...
              child: Column(
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      Expanded(
                        // showing total contents[index].items?.length to contents[index].topicsCount on 19-12-2025
                        // child: horizontalChapterItem(green77(), AppAssets.categorySvg, contents[index].title ?? '', '${contents[index].items?.length ?? 0} ${appText.lessons}', () {
                        child: horizontalChapterItem(green77(), AppAssets.categorySvg, contents[index].title ?? '', '${contents[index].topicsCount ?? 0} ${appText.lessons}', () {
                          contents[index].isOpen = !(contents[index].isOpen);
                          setState();
                        }, transparentColor: true, isFixWidth: true, iconColor: Colors.white),
                      ),
                      space(0, width: 12),
                      SvgPicture.asset(
                              ...
                              ...List.generate(contents[index].items?.length ?? 0, (i) {
                                return horizontalChapterItem(
                                  ...
                                  if (contents[index].items![i].type == 'assignment') {
                                    ...
                                    await nextRoute(AssignmentsPage.pageName, arguments: [
                                    ...
                                    await nextRoute(QuizzesPage.pageName, arguments: [
                                      contents[index].items![i],
                                      courseId,
                                      previousLink,
                                      i,
                                    ...
                                    await nextRoute(SingleContentPage.pageName, arguments: [
                                      contents[index].items![i],
                                      courseId,
                                      previousLink,
                                      i,
      ...
      child: Column(
        children: [
          ...
          ...List.generate(courseData!.quizzes.length, (i) {
            return horizontalChapterItem(green77(), AppAssets.shieldSvg, courseData.quizzes[i].title ?? '', '${courseData.quizzes[i].questionCount} ${appText.questions} | ${courseData.quizzes[i].time} ${appText.min}', () {
      ...
      child: Column(
        children: [
          ...
          ...List.generate(courseData!.certificates.length, (i) {
            return horizontalChapterItem(orange50, AppAssets.certificates2Svg, courseData.certificates[i].title ?? '', timeStampToDate((courseData.certificates[i].createdAt ?? 0) * 1000), () {
    ...
    baseBottomSheet(
      ...
      child: Column(
        ...
        children: [
            ...
            child: Row(
              children: [
                ClipRRect(
                  ...
                  child: fadeInImage(notice.creator?.avatar ?? '', 40, 40),
                ),
                space(0, width: 5),
                Expanded(
                  child: Column(
          ...
          button(onTap: backRoute, width: getSize().width, height: 52, text: appText.close, bgColor: green77(), textColor: Colors.white),
  ...
  static Future forumOptionSheet(Can can, bool isPin, Function onTapPin, Function onTapMarkAsResolved, Function onTapEdit) async {
    return await baseBottomSheet(child: Builder(
      builder: (context) {
        return Padding(
          padding: padding(),
          child: Column(
                        ...
                        child: button(
                            onTap: () async {
                              if (titleController.text.trim().isNotEmpty && descController.text.trim().isNotEmpty) {
                                ...
                                bool res = await ForumService.newQuestion(courseId, titleController.text.trim(), descController.text.trim(), attachment);
  ...
  static Future forumReplaySheet(Forums? question, {bool isEdit = false, ForumAnswerModel? answer}) async {
    ...
    return await baseBottomSheet(child: StatefulBuilder(
