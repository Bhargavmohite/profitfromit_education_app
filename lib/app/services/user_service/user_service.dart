...
class UserService{
  ...
  static Future<List<NotificationModel>> getAllNotification({
    int offset = 0,
    int limit = 10,
    bool append = false,
  }) async {
    ...
    try {
      String url = '${Constants.baseUrl}panel/notifications?offset=$offset&limit=$limit';
      ...
      if (jsonResponse['success']) {
        ...
        if (append) {
          // Append new data to existing list in provider
          final currentList = locator<UserProvider>().notification;
          locator<UserProvider>().setNotification([...currentList, ...data]);
        } else {
          // Replace old list with new data
          locator<UserProvider>().setNotification(data);
        }
  ...
  static Future<bool> deleteAccount() async {
    try {
      var id = locator<UserProvider>().profile?.id.toString();
      String url = '${Constants.baseUrl}config/deleteregisteruser?id=$id';
      Response res = await httpGetWithToken(
          url,
          isRedirectingStatusCode: false
      );
  ...
  static Future<String> getLatestEvents()async{
    String data = "";
    try{
      String url = '${Constants.baseUrl}latest-active-event';
      ...
      if(jsonResponse['success']){
        data = jsonResponse["data"]["url"];
        debugPrint("latest event url =============> $data");
        return data;
      }else{
        ErrorHandler().showError(ErrorEnum.error, jsonResponse);
        return data;
